...
![Содержимое файла `s3dis_head_output.txt`:](area1txt.png)
...
Если данные хранятся в едином NumPy-массиве, где последний столбец является меткой класса, разделение выполняется элегантно с помощью срезов (slicing):```python
# 'dataset' - наш массив формы (N, 7)
# X - входные признаки, y - метки
X = dataset[:, :-1]  # Все строки, все столбцы кроме последнего
y = dataset[:, -1]   # Все строки, только последний столбец```
